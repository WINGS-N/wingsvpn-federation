...
func runOnce(ctx context.Context, cfg Config, picker *dialpick.Picker) error {
	...
	for {
		...
		case task := <-tasks:
			...
			// Круг заводит либо первая задача, либо просьба замерить сейчас.
			// Обычная пересылка списка целей идёт раз в минуту, и запускать по
			// ней замер значит гонять мегабайты через донора вместо интервала
			if (first || task.GetMeasureNow()) && !busy {
				busy = true
				go func(t *fedpb.ProbeTask) { measuring <- measureAll(ctx, cfg, t, ready) }(current)
			}
			if !first && !task.GetMeasureNow() && round == nil {
