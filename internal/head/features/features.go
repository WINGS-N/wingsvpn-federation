...
type Vector struct {
	...
	SpreadHours float64
	// RandomNameShare - доля имён, похожих на высранные генератором. Ловит
	// малварь без всякого списка, по одной только форме имени
...
func build(subjectID string, window time.Duration, sightings []Sighting, ports []PortHit) Vector {
	...
	if v.Domains > 0 {
		v.RandomNameShare = float64(randomNames) / float64(v.Domains)
	}
