...
// RestoreNodes поднимает накопленное по нодам из общего места.
//
// Зовётся один раз при старте, до того как придёт первый отчёт: иначе реплика,
// поднятая с пустой памятью, покажет ноль там, где нода возила гигабайты
func (a *Aggregator) RestoreNodes(store SharedStore) error {
	...
	defer a.mu.Unlock()
	if a.seed == nil {
		a.seed = make(map[string]NodeShare, len(rows))
	}
	for _, row := range rows {
		if _, live := a.nodes[row.NodeID]; live {
			continue
		}
		a.seed[row.NodeID] = row
	}
	return nil
}
